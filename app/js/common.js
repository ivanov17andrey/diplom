...
document.getElementById('ranging-send').addEventListener('click', function() {
	...
	if(input.value.trim() != ""){
		...
		document.getElementById('comparition__box').classList.toggle('hidden');
		alert('Спасибо!')
	}else{
		input.value = '';
		alert('Введите имя');
...
let result;
document.getElementById('emptydb').addEventListener('click', function() {
	$.ajax({
		url: 'http://a91756we.beget.tech/emptydb.php?defeatCache=' + rand(),
		type: 'POST',
		// data: requestData,
		// contentType: 'application/json',
		//dataType: 'json',
		success: function() {
			alert('База данных была очищена');
		}
	});
})
...
function receiveData(){
	$.ajax({
		type: 'GET',
		url:'algoritm.php?defeatCache=' + rand(),
...
// let alternatives = ['Bell 429 WLG', 'Bell 407 GXP', 'Airbus AS365 N3+'];
let name;
...
function showNextQuestion() {
	...
	if (iteration != numOfAlternatives) {
		...
		document.getElementById('equal').textContent = `${alternatives[row]} равноценны  ${alternatives[col]}`;
...
function showMatrixPair(matrixPair) {
	let el = document.getElementById('matrix-box');
	el.innerHTML = '';
	let h2 = document.createElement('h2');
	h2.textContent = 'Матрица смежности';
	let table = document.createElement('table');
	table.id = 'pair__matrix';
	table.className = 'pair__matrix';
	el.appendChild(h2);
	table = el.appendChild(table);
	// table = document.getElementById('pair__matrix');
	for (let i = 0; i < matrixPair.length; i++) {
		let tableRow = table.appendChild(document.createElement('tr'));
		for (let j = 0; j < matrixPair.length; j++) {
			let cellValue = matrixPair[i][j];
